...
export function apply(ctx, config) {
    ...
    if (config.listen.enabled) {
        ...
        void startDirectServer(ctx, {
            ...
            .catch((error) => {
            ctx.logger.error(`dsh-companion bridge: 直连 server 启动失败 (${config.listen.host}:${config.listen.port}): ${error instanceof Error ? error.message : String(error)}`);
        });
    ...
    ctx.logger.info(`dsh-companion bridge ready: worker="${workerName}" caps=${config.caps} ` +
