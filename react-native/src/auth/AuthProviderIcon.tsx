...
type Props = Readonly<{
  ...
  onPress: () => void;
  /**
   * `icon`：方形描边圆钮 + 下方文字（次级页面的紧凑排布）
   * `button`：整行描边按钮（登录首屏，图标 + 居中文案）
   */
  variant?: 'icon' | 'button';
}>;

/** 第三方登录入口（RNR Button + 品牌 SVG）。 */
export function AuthProviderIcon({
  ...
  if (variant === 'button') {
    return (
      ...
      >
        <View className="mr-2">
          <BrandMark name={name} color={foreground ?? '#000000'} size={18} />
        </View>
        <Text className="text-sm font-medium">使用 {label} 登录</Text>
      </Button>
    );
  }
  return (
    <View className="items-center gap-1">
      <Button
        ...
        variant="outline"
      >
        <BrandMark name={name} color={foreground ?? '#000000'} />
      </Button>
...
function BrandMark({
  ...
  color,
  size = 24,
}: Readonly<{ name: AuthProviderName; color: string; size?: number }>) {
  if (name === 'google') {
    return (
