...
/** 组装二维码负载（Worker 侧使用）。 */
export function encodePairQr(payload) {
...
export function parsePairQr(text) {
    const raw = text.trim();
    if (raw.length === 0)
        return null;
    ...
    if (!raw.startsWith(prefix)) {
        ...
        if (!withoutSlashes.startsWith(`${PAIR_QR_HOST}?`))
            return null;
...
function fromQuery(query) {
    ...
    if (gw !== undefined && gw.length > 0)
        payload.gatewayUrl = gw;
    return payload;
}
/** 直连 WebSocket 地址（同网路径）；host 缺失时为 null。 */
export function pairDirectWsUrl(payload) {
