...
function resolveDshBin(explicit) {
  ...
  if (fromEnv !== void 0 && fromEnv.length > 0 && existsSync(fromEnv)) return fromEnv;
  ...
  if (existsSync2(manifestPath)) {
    try {
      ...
      if (!bundles.includes("@deepseek-ai/dsh-web-app")) {
        ...
        writeFileSync(manifestPath, `${JSON.stringify(existing, void 0, 2)}
`);
      }
    } catch {
    }
...
function writeWebUrl(url) {
  ...
  try {
    const stored = JSON.parse(readFileSync2(file, "utf8"));
    if (url === void 0) delete stored["webUrl"];
    else stored["webUrl"] = url;
    writeFileSync2(file, `${JSON.stringify(stored, void 0, 2)}
`);
  } catch {
  }
}
...
function isRunning() {
  if (!existsSync3(pidFile())) return null;
  const pid = Number.parseInt(readFileSync2(pidFile(), "utf8").trim(), 10);
...
async function supervise(dshBin, args, env, info) {
  writeFileSync2(pidFile(), `${process.pid}
`);
  writeFileSync2(runInfoFile(), `${JSON.stringify({ ...info, pid: process.pid, startedAt: Date.now() }, void 0, 2)}
`);
  let stopping = false;
  let child;
  const stop = (signal) => {
  ...
  const flagTimer = setInterval(() => {
    if (existsSync3(`${dshcDir()}/${STOP_FLAG}`)) stop("SIGTERM");
  }, 2e3);
  flagTimer.unref();
  let backoffMs = 1e3;
  while (!stopping) {
...
function requestStop() {
  writeFileSync2(`${dshcDir()}/${STOP_FLAG}`, "1\n");
}
