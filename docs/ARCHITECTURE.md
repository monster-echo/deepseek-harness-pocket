...
电脑 ×N = Worker (packages/dsh-companion-bridge/)
...
- `packages/dsh-bridge-protocol`：纯 TS 类型 + codec，全系统唯一协议真相源（app / 插件 / gateway / e2e 共享；零 Node/DOM API）
- `packages/dsh-companion-bridge`：peer 依赖 cordis；导出插件 + `dshc` bin
...
- 详见 `packages/dsh-bridge-protocol/src/`
