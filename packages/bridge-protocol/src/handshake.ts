...
export const M1_CAPABILITIES: BridgeCapabilities = {
...
export interface HandshakeArgs {
  readonly client: 'deepseek-harness-pocket-app' | 'fake-phone' | string
...
export function handshakeArgs(client: string): HandshakeArgs {
  return { client, protocolVersion: PROTOCOL_VERSION }
}

export function parseHandshakeArgs(value: unknown): HandshakeArgs | null {
  if (typeof value !== 'object' || value === null) return null
  const v = value as Record<string, unknown>
  if (typeof v.client !== 'string' || v.client.length === 0) return null
  if (typeof v.protocolVersion !== 'string') return null
