...
export function HomeShellScreen() {
  ...
  const activeSessionTitle = useDshStore(
  ...
  const subtitle = (() => {
    ...
    if (activeSessionId !== null) {
      ...
      const presetLabel =
        PRESET_LABELS[
          newSessionPreset.length > 0 ? newSessionPreset : "standard"
        ] ?? "标准";
      const permLabel =
        permission !== null
      ...
      >
        ...
        <View style={styles.topTitle}>
          {hasWorker ? (
            <>
              <Text
                style={[styles.workerName, { color: palette.text }]}
                numberOfLines={1}
              >
                {activeSessionTitle ?? activeWorker?.name ?? "掌鲸 DSH Pocket"}
              </Text>
              <Text
                style={[styles.workerSub, { color: palette.textSecondary }]}
                numberOfLines={1}
              >
                {subtitle}
              </Text>
            ...
            <View style={styles.brandRow}>
              ...
              />
              <Text style={[styles.workerName, { color: palette.text }]}>
                掌鲸 DSH Pocket
              </Text>
            </View>
          )}
  ...
  return (
    <View style={styles.empty}>
      ...
      <Pressable
        ...
        onPress={() => navigate("dsh.pair")}
      >
        <Text style={styles.emptyButtonText}>开始</Text>
      </Pressable>
    </View>
  );
}
