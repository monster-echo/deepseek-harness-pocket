...
export function buildDeviceLinkQr(input: {
  ...
  const parts: string[] = [`v=1`, `c=${encodeURIComponent(input.code)}`]
  if (input.name !== undefined && input.name.length > 0) parts.push(`h=${encodeURIComponent(input.name)}`)
  if (input.platform !== undefined && input.platform.length > 0) parts.push(`p=${encodeURIComponent(input.platform)}`)
  if (input.gateway !== undefined && input.gateway.length > 0) parts.push(`gw=${encodeURIComponent(input.gateway)}`)
 ...
 */
export function parseDeviceLinkQr(text: string): DeviceLinkQrPayload | null {
  const trimmed = text.trim()
  if (!trimmed.toLowerCase().startsWith(DEVICE_LINK_SCHEME)) return null
  const queryStart = trimmed.indexOf('?')
  if (queryStart < 0) return null
  const params = parseQuery(trimmed.slice(queryStart + 1))
  const code = (params['c'] ?? '').toUpperCase()
  if (!isDeviceLinkCode(code)) return null
...
export function isDeviceLinkCode(code: string): boolean {
  ...
  for (const ch of code) {
    if (!DEVICE_LINK_ALPHABET.includes(ch)) return false
  }
  return true
}
...
export function buildDeviceLinkCredential(code: string, secret: string): string {
  return `${DEVICE_LINK_CREDENTIAL_PREFIX}${code}.${secret}`
}

/** 拆解桌面端凭据；格式不对返回 null。 */
export function parseDeviceLinkCredential(
  credential: string,
): { code: string; secret: string } | null {
  if (!credential.startsWith(DEVICE_LINK_CREDENTIAL_PREFIX)) return null
