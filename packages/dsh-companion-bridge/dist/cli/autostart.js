...
const LABEL = 'com.rwecho.dshcompanion.worker';
export function autostartInstall(gatewayUrl) {
    if (process.platform === 'darwin')
        return installLaunchd(gatewayUrl);
    if (process.platform === 'linux')
        return installSystemd(gatewayUrl);
...
export function autostartUninstall() {
    ...
    if (process.platform === 'linux') {
        const target = `${homedir()}/.config/systemd/user/dsh-companion.service`;
        spawnSync('systemctl', ['--user', 'disable', '--now', 'dsh-companion.service'], { stdio: 'ignore' });
        rmSync(target, { force: true });
...
function installLaunchd(gatewayUrl) {
    ...
    const log = `${homedir()}/.dsh-companion/launchd.log`;
...
`;
    ...
    if (loaded.status !== 0)
        return `已写入 ${target}，但 launchctl load 失败，请手动加载`;
...
`;
    ...
    const enabled = spawnSync('systemctl', ['--user', 'enable', '--now', 'dsh-companion.service']);
    if (enabled.status !== 0)
        return `已写入 ${target}，但 enable 失败，请手动 systemctl --user enable --now dsh-companion`;
